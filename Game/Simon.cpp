...
void CSimon::Render()
{
	...
	else if (onStairs != 0)
	{
		...
		{
			if (onStairs == 1)
				ani = SIMON_ANI_GO_UPSTAIR;
			else if (onStairs == -1)
				ani = SIMON_ANI_GO_DOWNSTAIR;
		}
		else
		{
			if (onStairs == 1)
...
void CSimon::GoUpStair()
{
	...
	if (onStairs == 0)
	{
		...
		{
			if (dynamic_cast<CStartStair*>(ovObjects[i]) || dynamic_cast<CVariousStair*>(ovObjects[i]))
			{				
				stairs = ovObjects[i];
...
void CSimon::ProceedOnStairs()
{
	...
	if (onStairs == 1)
	{
		for (UINT i = 0; i < ovObjects.size(); ++i)
		{
			...
			if (dynamic_cast<CEndStair*>(ovObjects[i]) || ((dynamic_cast<CVariousStair*>(ovObjects[i]))&& this->nx ==-1))
				{					
					stairs = ovObjects[i];
					break;
				}
	...
	{
		for (UINT i = 0; i < ovObjects.size(); ++i)
			if (dynamic_cast<CStartStair*>(ovObjects[i]) || 
...
void CSimon::GoDownStair()
{
	...
	if (onStairs == 0)
	{
		...
		for (UINT i = 0; i < ovObjects.size(); ++i)
		{
			if (dynamic_cast<CEndStair *>(ovObjects[i]) || 
